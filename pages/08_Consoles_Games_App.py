...
X_data = pd.read_csv("data/consoles_games_Data.csv")
y_data = pd.read_csv("target/y_consoles_games.csv")
model = RandomForestRegressor(n_jobs = -1, random_state = 42)

# =========================splitting the dataset================================
X_consoles_games = X_data.drop(columns = ["unit_price", "volume"], axis = 1)

X_train, X_test, y_train, y_test = train_test_split(X_consoles_games, y_data, test_size=0.2,random_state=15)
...
def consoles_games():
  st.sidebar.image("images/price_image7.jfif")
  st.markdown("""
           <style>
           .text-header{
            color :#0B1B44;
           ...
           <p class = text-header >Consoles Games Price Optimization App</p>
