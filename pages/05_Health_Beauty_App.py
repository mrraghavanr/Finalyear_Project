...
X_data = pd.read_csv("data/health_beauty_Data.csv")
y_data = pd.read_csv("target/y_health_beauty.csv")
model = RandomForestRegressor(n_jobs = -1, random_state = 42)

# =========================splitting the dataset================================
X_health_beauty = X_data.drop(columns = ["unit_price"], axis = 1)

X_train, X_test, y_train, y_test = train_test_split(X_health_beauty, y_data, test_size=0.2,random_state=42)
...
def health_beauty():
  st.sidebar.image("images/price_image8.jfif")
  st.markdown("""
           <style>
           .text-header{
            color :#BF5EA3;
           ...
           <p class = text-header>Health Beauty Price Optimization App</p>
