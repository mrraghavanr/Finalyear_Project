...
X_data = pd.read_csv("data/computers_accessories_Data.csv")
y_data = pd.read_csv("target/y_computers_accessories.csv")
model = RandomForestRegressor(n_jobs = -1, random_state = 42)

# =========================splitting the dataset================================
X_computers_accessories = X_data.drop(columns = ["unit_price", "product_photos_qty"], axis = 1)

X_train, X_test, y_train, y_test = train_test_split(X_computers_accessories, y_data, test_size=0.2,random_state=50)
...
def computers_accessories():
  st.sidebar.image("images/price_image6.jfif")
  st.markdown("""
           <style>
           .text-header{
            color :#1E465F;
           ...
           <p class = text-header >Computers Accessories Price Optimization App</p>
