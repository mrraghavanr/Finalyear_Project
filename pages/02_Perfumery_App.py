...
X_data = pd.read_csv("data/perfumery_Data.csv")
y_data = pd.read_csv("target/y_perfumery.csv")
model = RandomForestRegressor(n_jobs = -1, random_state = 42)

# =========================splitting the dataset================================
X_perfumery = X_data.drop(columns = ["unit_price"], axis = 1)

X_train, X_test, y_train, y_test = train_test_split(X_perfumery, y_data, test_size=0.2,random_state=250)
...
def perfumery():
  st.sidebar.image("images/price_image3.jfif")
  st.markdown("""
          <style>
          .text-header{
          color :#34211A;
          ...
          <p class = text-header>Perfumery Price Optimization App</p>
