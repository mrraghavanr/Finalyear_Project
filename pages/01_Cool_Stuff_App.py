...
X_data = pd.read_csv("data/cool_stuff_Data.csv")
y_data = pd.read_csv("target/y_cool_stuff.csv")
model = RandomForestRegressor(n_jobs = -1, random_state = 42)

# =========================splitting the dataset================================
X_cool_stuff = X_data.drop(columns = ["unit_price"], axis = 1)

X_train, X_test, y_train, y_test = train_test_split(X_cool_stuff, y_data, test_size=0.2,random_state=45)
...
def cool_stuff():
  st.sidebar.image("images/price_image4.jfif")
  st.markdown("""
           <style>
           .text-header{
            color :#405716;
           ...
           <p class = text-header>Cool Stuff Price Optimization App</p>
