...
X_data = pd.read_csv("data/furniture_decor_Data.csv")
y_data = pd.read_csv("target/y_furniture_decor.csv")
model = RandomForestRegressor(n_jobs = -1, random_state = 50)

# =========================splitting the dataset================================
X_furniture_decor = X_data.drop(columns = ["unit_price"], axis = 1)

X_train, X_test, y_train, y_test = train_test_split(X_furniture_decor, y_data, test_size=0.2,random_state=10)
...
def furniture_decor():
  st.sidebar.image("images/price_image10.jfif")
  st.markdown("""
           <style>
           .text-header{
            color :#4D4541;
           ...
           <p class = text-header >Furniture Decor Price Optimization App</p>
