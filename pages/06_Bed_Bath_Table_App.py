...
X_data = pd.read_csv("data/bed_bath_table_Data.csv")
y_data = pd.read_csv("target/y_bed_bath_table.csv")
model = RandomForestRegressor(n_jobs = -1, random_state = 42)

# =========================splitting the dataset================================
X_bed_bath_table = X_data.drop(columns = ["unit_price"], axis = 1)

X_train, X_test, y_train, y_test = train_test_split(X_bed_bath_table, y_data, test_size=0.2,random_state=250)
...
def bed_bath_table():
  st.sidebar.image("images/price_image2.jpg")
  st.markdown("""
           <style>
           .text-header{
            color :#CEA472;
           ...
           <p class = text-header>Bed-Bath-Table Price Optimization App</p>
