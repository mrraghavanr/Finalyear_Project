...
X_data = pd.read_csv("data/watches_gifts_Data.csv")
y_data = pd.read_csv("target/y_watches_gifts.csv")
model = RandomForestRegressor(n_jobs = -1, random_state = 42)

# =========================splitting the dataset================================
X_watches_gifts = X_data.drop(columns = ["unit_price"], axis = 1)

X_train, X_test, y_train, y_test = train_test_split(X_watches_gifts, y_data, test_size=0.2,random_state=50)
...
def watches_gift():
  st.sidebar.image("images/price_image9.jpg")
  st.markdown("""
           <style>
           .text-header{
            color :#11202A;
           ...
           <p class = text-header >Watches Gifts Price Optimization App</p>
